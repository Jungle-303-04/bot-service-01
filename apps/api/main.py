    ...
    try:
        async with current_pool().acquire() as conn:
            async with conn.transaction():
                await conn.execute("SELECT * FROM inventory_items WHERE sku = 'checkout-main' FOR UPDATE")
...
async def observe_requests(request: Request, call_next):
    ...
    try:
        ...
        if scenarios["error_spike"] and path.startswith("/api/") and random.random() < 0.35:
            ...
            status = 503
            return JSONResponse({"detail": "intentional checkout failure spike"}, status_code=503)
...
async def orders() -> dict[str, Any]:
    if APP_FLAVOR == "bad-schema":
        await timed_db("bad_schema", "SELECT missing_checkout_column FROM orders LIMIT 1")
    rows = await timed_db(
        "orders",
        """
...
async def create_order() -> dict[str, Any]:
    ...
    async with current_pool().acquire() as conn:
        async with conn.transaction():
            ...
            await conn.execute("UPDATE inventory_items SET available = available - 1, updated_at = now() WHERE sku = 'checkout-main'")
