...
function splitTreemap(count: number, x = 0, y = 0, w = 100, h = 100, seed = 13, start = 0): Tile[] {
  if (count <= 0) return [];
  if (count === 1) {
    const heat = ((start * 37 + seed * 19) % 100) / 100;
    return [{ x, y, w, h, heat, index: start }];
  }

  const bias = 0.42 + (((seed * 97 + count * 13) % 20) / 100);
  ...
  if (w >= h) {
    ...
    return [
      ...splitTreemap(first, x, y, w1, h, seed + 3, start),
      ...splitTreemap(second, x + w1, y, w - w1, h, seed + 7, start + first),
    ];
  }

  const h1 = (h * first) / count;
  return [
    ...splitTreemap(first, x, y, w, h1, seed + 5, start),
    ...splitTreemap(second, x, y + h1, w, h - h1, seed + 11, start + first),
  ];
}
...
function loadLabel(count: number) {
  if (count <= 1) return '단일 서비스 블록';
  return `${formatCount(count)}개 실제 저장 셀`;
}
...
export default function App() {
  ...
  const storedRows = status?.rows.telemetry_samples ?? 0;
  ...
  const startLoad = async () => {
    setBusy('부하 시작');
    setBaselineRows(status?.rows.telemetry_samples ?? 0);
    setConfirmedWork(0);
    setRunning(true);
    setNotice('실제 요청을 저장하며 분할 중');
  ...
  return (
    <main className="shell">
      ...
      <section className="metrics">
        <Metric label="실제 저장 셀" value={`${formatCount(visibleCells)} / ${formatCount(targetCells)}`} />
      ...
      <section className="heat-card" style={{ ['--heat' as string]: pressure }}>
        ...
        <div className="heat-map" aria-label="실제 저장 부하 히트맵">
          {tiles.map((tile) => (
            <i
              key={tile.index}
              style={{
