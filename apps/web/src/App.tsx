  ...
  useEffect(() => {
    ...
    timerRef.current = window.setInterval(() => {
      ...
      void Promise.allSettled(plans.map((botIndex) => api<WorkResult>('/api/work', {
        method: 'POST',
        body: JSON.stringify({ bot_id: `checkout-bot-${botsRef.current[botIndex]?.id ?? botIndex + 1}`, failure_rate: failureRate }),
      }))).then((results) => {
        setBots((current) => {
  ...
  return (
    <main className="shell">
      ...
      <section className="footer-panel">
        ...
        <div>
          <span>DB 누적</span>
          <strong>{formatCount(status?.rows.telemetry_samples ?? 0)}건</strong>
        </div>
      </section>
    </main>
  );
