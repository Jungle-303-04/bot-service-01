...
export default function App() {
  ...
  const loadPressure = Math.min(1, activeIncidents * 0.18 + Math.max(desired - 2, 0) * 0.1 + (cluster?.template_flavor !== 'stable' ? 0.32 : 0));
  ...
  return (
    <main className="shell">
      ...
      <section className="panel load-panel" style={{ ['--load' as string]: loadPressure }}>
        <div className="panel-title">
          <strong>부하 확산</strong>
          <span>{loadCells.toLocaleString()} 요청 셀 · 실제 파드 {ready}/{desired}</span>
        </div>
